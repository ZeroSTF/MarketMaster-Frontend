...
export class OptionService {
  ...
  private readonly options: Signal<Option | null> = computed(() =>
    this.optionsignal?.()
  );
  ...
  buyOption(option: Option): Observable<Option> {
    ...
    return this.http.post<Option>(
      `${this.API_URL}/option/buyoption/${username}`,
      option
    );
  }
  getOptions(): Observable<Option[]> {
    ...
    return this.http.get<Option[]>(
      `${this.API_URL}/portf/myoptions/${username}`
    );
  }
  ...
  applyOption(option: Option): Observable<Transaction> {
    ...
    return this.http.post<Transaction>(
      `${this.API_URL}/option/applyoption/${username}`,
      option
    );
  }
